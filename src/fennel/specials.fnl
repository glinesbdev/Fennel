...
(set safe-require (fn [modname]
                    "This is a replacement for require for use in macro context.
        ...
        (let [mod ((load-code (.. "return " (. modexpr 1))))]
          (or (include-circular-fallback mod modexpr opts.fallback ast)
              ...
              (match (search-module mod) ; try fennel path first
                fennel-path
                (include-path ast opts fennel-path mod true)
                ;; then search for a lua module
                _
                (let [lua-path (search-module mod package.path)]
