...
(fn hashfn-arg-name [name multi-sym-parts scope]
  (if (not scope.hashfn) name ; don't do anything unless in a hashfn
      (= name "$") :$1 multi-sym-parts
      (do
        (when (and multi-sym-parts (= (. multi-sym-parts 1) "$"))
          (tset multi-sym-parts 1 :$1))
        (table.concat multi-sym-parts ".")) name))
...
if they have already been declared via declare-local"
  ...
  (let [name (. symbol 1)
        ...
        name (hashfn-arg-name name multi-sym-parts scope)]
...
(fn macroexpand* [ast scope once]
  ...
  ;; bail early if not a list
  (if (not (utils.list? ast))
      ast
...
(fn serialize-number [n]
  (pick-values 1 (-> (tostring n) (string.gsub "," "."))))
...
(fn compile-table [ast scope parent opts compile1]
  (let [buffer []]
    ...
    (let [keys (doto (utils.kvmap ast write-other-values)
      ...
      (utils.map keys
                 (fn [k]
                   (let [v (tostring (. (compile1 (. ast (. k 2)) scope parent
                                                  {:nval 1})
                                        1))]
