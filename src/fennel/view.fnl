...
(fn number->string [n]
  ...
  (pick-values 1 (-> n tostring (string.gsub "," "."))))

(fn colon-string? [s {: prefer-colon?} colon?]
  (and (= :string (type s)) (s:find "^[-%w?^_!$%&*+./@|<=>]+$")
       (if (not= colon? nil) colon?
           (= :function (type prefer-colon?)) (prefer-colon? x)
           prefer-colon?)))
