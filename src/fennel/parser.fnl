...
stream is finished."
  ...
  (fn parse-stream []
    ...
    (fn parse-loop [b]
      (if (not b) nil ; EOF
          (= b 59) (parse-comment (getb) [";"]) ; comment
          (= (type (. delims b)) :number) (open-table b) ; table open
          (. delims b) (close-table b) ; table close
          (= b 34) (parse-string b) ; string
          (. prefixes b) (parse-prefix b) ; sugared form
          (or (sym-char? b) (= b (string.byte "~"))) (parse-sym b) ; symbol
